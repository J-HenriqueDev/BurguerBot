...
const manageOrder = {
    ...
    async validateQuantity(message: Message, client: Whatsapp) {
        ...
        if (Number.isInteger(parseInt(message.body)) && parseInt(message.body) > 0) {
            ...
            client.sendButtons(
                message.from,
                'Você deseja adicionar um novo item ao pedido?',
...
const manageAddress = {
                        ...
                        if (cep.erro) responseError()
                            ...
                            if (cep.bairro === '') {
                                ...
                                sendMessage = `➠ *Cidade:* ${address.city}
                                    ...
                                    Digite agora o número da residência para a entrega:`.replace(/^ +/gm, '')
                                responseSuccess(sendMessage)
    ...
    async checkPublicPlace(message: Message, client: Whatsapp) {
        ...
        client.sendText(
            message.from,
            '*Digite agora o número da residência. (se houver)*\n\n*Ex:* 1254 ou S/N'
        )
            .then(result => client.stopTyping(message.from))
...
const manageChat = {
        ...
        if (message.type === 'list_response') {
            ...
            if (itemSelected) {
                ...
                client.sendButtons(
                    ...
                    .catch(err => console.log('Erro ao enviar mensagens de solicitação de quantidade\n--f openOrder: ', err))
            }
        }
    ...
    okOrder(message: Message, client: Whatsapp) {
        ...
        client.sendText(message.from, 'Ótimo😉!\nEntão vamos ao cadastro do seu endereço.')
            .then(result => {
